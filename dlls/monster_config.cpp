...
bool scan_monster_cfg( FILE* fp )
{
	char input[1024];
	char id[33];
	bool origin, angle, delay, angle_min, angle_max, monster, foundId;
	float x, y, z;
	...
	while( get_input( fp, input ) )
	{
		...
		if( input[0] == '{' )
		{
			origin = angle = delay = angle_min = angle_max = monster = foundId = FALSE;
			...
			while( get_input( fp, input ) )
			{
				...
				// TJ
				// Check if this entry has an id associated with it.
				// So we can spawn it via the "monster" command
				if( strncmp( input, "id/", 3 ) == 0 )
				{
					if( foundId )
					{
						META_CONS( "[MONSTER] ERROR: id found twice: %s", input );
						LOG_MESSAGE( PLID, "ERROR: id found twice: %s", input );
						return TRUE;  // error
					}

					if( sscanf( &input[3], "%s", &id ) != 1 )
					{
						META_CONS( "[MONSTER] ERROR: invalid id: %s", input );
						LOG_MESSAGE( PLID, "ERROR: invalid id: %s", input );
						return TRUE;  // error
					}

					strncpy( monster_spawnpoint[monster_spawn_count].id, id, 32 );
					foundId = TRUE;
				}
				else if( strncmp( input, "origin/", 7 ) == 0 )
				{
					if( origin )
					{
