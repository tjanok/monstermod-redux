...
IMPLEMENT_CUSTOM_SCHEDULES( CGargantua, CMBaseMonster );
...
void CGargantua::EyeUpdate( void )
{
	...
	{
		Vector angles;
		GetAttachment( 0, eyeAttachment, angles );
		...
		//UTIL_SetOrigin( m_pEyeGlow->pev, eyeAttachment );
	}
}
...
void CGargantua::FlameDamage( Vector vecStart, Vector vecEnd, entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int iClassIgnore, int bitsDamageType )
{
	...
	while( ( searchEnt = UTIL_FindEntityInSphere( pEntity->edict(), vecMid, searchRadius ) ) != NULL )
	{
		if( searchEnt->v.takedamage != DAMAGE_NO )
		{
...
void CGargantua::Spawn()
{
	...
	m_pEyeGlow->SetAttachment( edict(), 0 );
	//EyeOff();
	m_seeTime = gpGlobals->time + 5;
...
void CGargantua::DeathEffect( void )
{
	...
	*/
	//CSmoker *pSmoker = CreateClassPtr( ( ( CSmoker* ) NULL ) );
...
// players head)
//=========================================================
CMBaseEntity* CGargantua::GargantuaCheckTraceHullAttack( float flDist, int iDamage, int iDmgType )
{
	TraceResult tr;
...
void CGargantua::RunTask( Task_t* pTask )
{
	...
	case TASK_DIE:
		if( gpGlobals->time > m_flWaitFinished )
		{
			...
			for( i = 0; i < 10; i++ )
			{
				CMGib* pGib = GetClassPtr( ( CMGib* ) NULL );
